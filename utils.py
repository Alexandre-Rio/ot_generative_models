...
__all__ = ['plot_grid', 'generate_plot_grid', 'pairwise_cosine_distance', 'CReLU', 'sinkhorn_divergence']
...
def generate_plot_grid(generator, model='sinkhorn_gan', latent_dim=2, latent_space='uniform', img_size=32, size=8, save=False):
    """
    Generate a fake image grid (size x size) to visualize the data generated by the model
    """
    # Generate fake data
    if latent_space == 'uniform':
        z = torch.rand(size * size, latent_dim)
        if model == 'ot_gan':
            z = 2 * z - 1
    elif latent_space == 'gaussian':
        z = torch.randn(size * size, latent_dim)
    else:
        z = None

    if generator.is_cuda:
        z = z.cuda()
    output = generator(z.cuda())
