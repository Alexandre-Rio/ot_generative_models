...
def generate_plot_grid(generator, parameters, img_size=32, size=8, save=False):
    ...
    """
    # Generate fake data
    if parameters.latent_space == 'uniform':
        z = torch.rand(size * size, parameters.latent_dim)
        if parameters.model == 'ot_gan':
            z = 2 * z - 1
    elif parameters.latent_space == 'gaussian':
        z = torch.randn(size * size, parameters.latent_dim)
    else:
        z = None
